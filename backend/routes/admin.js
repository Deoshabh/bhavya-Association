...
router.get("/dashboard", auth, adminAuth, async (req, res) => {
  try {
    ...
    for (const category of categories) {
      listingStats.byCategory[category] = await Listing.countDocuments({
        category,
      });
    }
    ...
    const referralStats = {
      ...
      totalUsersWithReferrals: await User.countDocuments({
    ...
    const topReferrers = await User.find({
      ...
      .limit(10)
      .select(
        "name phoneNumber referralStats referralPerks.currentTier referralCode createdAt"
      );
    ...
    const recentReferrals = await Referral.find({})
      ...
      .select(
        "referrer referred status referralDate completionDate rewardGiven"
      );
...
router.get("/users/:userId", auth, adminAuth, async (req, res) => {
  try {
    ...
    const userListings = await Listing.find({ user: req.params.userId }).select(
...
router.put("/users/:userId", auth, adminAuth, async (req, res) => {
  try {
    const { planType, accountStatus, isPublic, name, occupation } = req.body;
...
router.put("/listings/:listingId", auth, adminAuth, async (req, res) => {
  try {
    const { title, category, description, approved } = req.body;
...
router.get("/qa/stats", auth, adminAuth, async (req, res) => {
  try {
    ...
    const reportedContent =
      (await Question.countDocuments({ status: "reported" })) +
...
router.put("/questions/:id", auth, adminAuth, async (req, res) => {
  try {
    const { status, featured, priority } = req.body;
...
router.put("/answers/:id", auth, adminAuth, async (req, res) => {
  try {
    const { status, isBestAnswer } = req.body;
...
router.get("/referrals", auth, adminAuth, async (req, res) => {
  try {
    ...
    const referrals = await Referral.find(query)
      ...
      .populate(
        "referrer",
        "name phoneNumber referralCode referralPerks.currentTier"
      )
...
router.get("/referrals/analytics", auth, adminAuth, async (req, res) => {
  try {
    ...
    const overallStats = {
      ...
      cancelledReferrals: await Referral.countDocuments({
        status: "cancelled",
      }),
      rewardsGiven: await Referral.countDocuments({ rewardGiven: true }),
      conversionRate: 0,
    ...
    if (overallStats.totalReferrals > 0) {
      overallStats.conversionRate = (
        (overallStats.completedReferrals / overallStats.totalReferrals) *
        100
      ).toFixed(2);
    }
    ...
    const topReferrers = await User.find({
      ...
      .limit(20)
      .select(
        "name phoneNumber referralStats referralPerks referralCode createdAt"
      )
      .lean();
...
router.get("/referrals/leaderboard", auth, adminAuth, async (req, res) => {
  try {
    ...
    const leaderboard = await User.find({
      ...
      .limit(parseInt(limit))
      .select(
        "name phoneNumber referralStats referralPerks referralCode createdAt accountStatus"
      )
      .lean();
 ...
 */
router.post(
  "/referrals/regenerate-code/:userId",
  auth,
  adminAuth,
  async (req, res) => {
    try {
 ...
 */
router.post(
  "/referrals/manual-reward/:userId",
  auth,
  adminAuth,
  async (req, res) => {
    try {
...
/**
 * @route   GET /api/admin/matrimonial/stats
 * @desc    Get matrimonial statistics for admin dashboard
 * @access  Admin
 */
router.get("/matrimonial/stats", auth, adminAuth, async (req, res) => {
  try {
    const stats = {
      total: await MatrimonialProfile.countDocuments({}),
      pending: await MatrimonialProfile.countDocuments({ status: "pending" }),
      approved: await MatrimonialProfile.countDocuments({ status: "approved" }),
      rejected: await MatrimonialProfile.countDocuments({ status: "rejected" }),
      hidden: await MatrimonialProfile.countDocuments({ status: "hidden" }),
      active: await MatrimonialProfile.countDocuments({ isActive: true }),
      inactive: await MatrimonialProfile.countDocuments({ isActive: false }),
    ...
    console.error("Admin matrimonial stats error:", err);
    res.status(500).json({ msg: "Server error" });
  }
});

/**
 * @route   GET /api/admin/matrimonial/profiles
 * @desc    Get all matrimonial profiles with filtering for admin
 * @access  Admin
 */
router.get("/matrimonial/profiles", auth, adminAuth, async (req, res) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    const profiles = await MatrimonialProfile.find(query)
      .populate("user", "name phoneNumber planType")
    ...
    console.error("Admin matrimonial profiles list error:", err);
    res.status(500).json({ msg: "Server error" });
  }
});

/**
 * @route   GET /api/admin/matrimonial/profiles/:id
 * @desc    Get detailed matrimonial profile for admin
 * @access  Admin
 */
router.get("/matrimonial/profiles/:id", auth, adminAuth, async (req, res) => {
  try {
    const profile = await MatrimonialProfile.findById(req.params.id)
      .populate("user", "name phoneNumber planType createdAt")
      .populate("approvedBy", "name");
    ...
    console.error("Admin matrimonial profile detail error:", err);
    res.status(500).json({ msg: "Server error" });
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      console.error("Admin matrimonial profile approval error:", err);
      res.status(500).json({ msg: "Server error" });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        msg: `Profile ${profile.isPublic ? "shown" : "hidden"} successfully`,
        profile,
      });
    } catch (err) {
      console.error("Admin matrimonial profile visibility toggle error:", err);
      res.status(500).json({ msg: "Server error" });
    }
  }
);
...
/**
 * @route   PUT /api/admin/matrimonial/profiles/:id/toggle-active
 * @desc    Toggle profile active status
 * @access  Admin
 */
router.put(
  "/matrimonial/profiles/:id/toggle-active",
  auth,
  adminAuth,
  async (req, res) => {
    try {
      ...
      console.error("Admin matrimonial profile active toggle error:", err);
      res.status(500).json({ msg: "Server error" });
    }
  }
);
...
/**
 * @route   PUT /api/admin/matrimonial/profiles/:id
 * @desc    Update matrimonial profile notes
 * @access  Admin
 */
router.put("/matrimonial/profiles/:id", auth, adminAuth, async (req, res) => {
  try {
    ...
    console.error("Admin matrimonial profile update error:", err);
    res.status(500).json({ msg: "Server error" });
  }
});

/**
 * @route   DELETE /api/admin/matrimonial/profiles/:id
 * @desc    Delete matrimonial profile
 * @access  Admin
 */
router.delete(
  "/matrimonial/profiles/:id",
  auth,
  adminAuth,
  async (req, res) => {
    try {
      ...
      console.error("Admin matrimonial profile deletion error:", err);
      res.status(500).json({ msg: "Server error" });
    }
  }
);
