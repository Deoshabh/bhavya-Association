  ...
  res.json({
    ...
    envVars: {
      ...
      JWT_SECRET: hasJwtSecret
        ? "Configured"
        : "NOT CONFIGURED - THIS IS REQUIRED",
    },
  ...
  try {
    ...
    try {
      ...
      if (validationError.name === "ValidationError") {
        const errors = Object.keys(validationError.errors).reduce(
          (acc, key) => {
    ...
    if (referrer) {
      try {
        ...
        console.log(
          `Referral completed: ${referrer.name} referred ${user.name}`
        );
      } catch (referralError) {
...
router.post("/login", async (req, res) => {
  ...
  console.log("Login attempt received:", {
    phoneNumber: req.body.phoneNumber
      ? `${req.body.phoneNumber.slice(0, 3)}...`
      : "missing",
  ...
  try {
    ...
    if (!user) {
      console.log(
        `Login failed: No user found with phone number starting with ${phoneNumber.substring(
          0,
          3
        )}`
      );
      return res
        .status(400)
        .json({ msg: "Invalid credentials", field: "phoneNumber" });
    }
    ...
    if (!isMatch) {
      console.log(
        `Login failed: Invalid password for user ${user.id} (${user.name})`
      );
      return res
        .status(400)
        .json({ msg: "Invalid credentials", field: "password" });
    }
...
router.post("/admin-login", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      console.log(
        `Admin login failed: No user found with phone number starting with ${phoneNumber.substring(
          0,
          3
        )}`
      );
    ...
    if (user.planType !== "admin") {
      console.log(
        `Admin login failed: User ${user.id} (${user.name}) is not an admin`
      );
      return res
        .status(403)
        .json({
    ...
    if (!isMatch) {
      console.log(
        `Admin login failed: Invalid password for admin user ${user.id} (${user.name})`
      );
      return res
        .status(400)
        .json({ msg: "Invalid credentials", field: "password" });
