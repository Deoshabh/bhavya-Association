  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center">
            <div className={`p-3 rounded-lg ${getTierColor(referralData?.referralPerks?.currentTier).replace('text-', 'bg-').replace('-600', '-100')}`}>
              {getTierIcon(referralData?.referralPerks?.currentTier)}
            </div>
            <div className="ml-4">
              <p className="text-sm font-medium text-gray-600">Current Tier</p>
              <p className={`text-lg font-bold capitalize ${getTierColor(referralData?.referralPerks?.currentTier)}`}>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-medium">
                    {referralData.referralStats.successfulReferrals} / {referralData.tierInfo.next.minReferrals}
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2 mb-4">
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min(100, (referralData.referralStats.successfulReferrals / referralData.tierInfo.next.minReferrals) * 100)}%` 
                    }}
                  ></div>
                </div>
                <p className="text-sm text-gray-600">
                  {referralData.tierInfo.next.minReferrals - referralData.referralStats.successfulReferrals} more referrals needed
                </p>
              </div>
            ) : (
              <div className="text-center py-4">
                <Trophy className="h-12 w-12 text-yellow-500 mx-auto mb-2" />
                <p className="text-lg font-semibold text-gray-900">Maximum Tier Reached!</p>
                <p className="text-gray-600">You've achieved the highest tier</p>
              </div>
            )}
          </div>
          ...
          {/* Unlocked Features */}
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Unlocked Features</h3>
            <div className="space-y-3">
              {referralData?.referralPerks?.unlockedFeatures?.map((feature, index) => (
            ...
            {referralData?.referralPerks?.specialBadges?.length > 0 && (
              <div className="mt-6">
                <h4 className="font-medium text-gray-900 mb-3">Special Badges</h4>
                <div className="flex flex-wrap gap-2">
                  {referralData.referralPerks.specialBadges.map((badge, index) => (
                    <span 
                      key={index}
      ...
      {activeTab === 'progress' && (
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Tier System</h3>
          <div className="space-y-4">
            {referralData?.tierInfo?.all?.map((tier, index) => {
              const isActive = tier.tier === referralData.referralPerks.currentTier;
              const isCompleted = referralData.referralStats.successfulReferrals >= tier.minReferrals;
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className={`p-2 rounded-lg mr-4 ${getTierColor(tier.tier)}`}>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          {referralData?.recentReferrals?.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400">Start sharing your referral link to see your referrals here</p>
            </div>
          )}
        </div>
      )}
